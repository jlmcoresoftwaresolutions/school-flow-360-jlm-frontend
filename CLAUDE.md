...
- Apenas dois addons do Storybook estão instalados: `@storybook/addon-docs` (autodocs) e `@storybook/addon-a11y` (checagens de acessibilidade), configurados em `.storybook/main.ts`. **Não adicione addons `@storybook/addon-vitest`/Chromatic/MCP** sem pedido explícito — o `storybook init` inclui esses por padrão, mas eles trazem junto os binários de navegador do Vitest + Playwright puramente para testes de componente, o que está fora do escopo (não há test runner de componentes neste projeto). Isso é diferente da dependência de dev avulsa `playwright` descrita no início deste arquivo, que existe apenas para verificação manual via screenshot, não para testes de componente.
...
- `.husky/commit-msg` executa `scripts/validate-commit-msg.mjs`, que rejeita o commit se a linha de assunto não começar com `feat:`/`fix:`/`docs:`/`refactor:` (opcionalmente `type(scope):`) ou exceder 72 caracteres. É um pequeno script Node independente, não o commitlint — se um novo tipo de commit precisar ser permitido, edite o array `allowedTypes` lá.
- Os scripts em `scripts/` rodam no Node, não no navegador, então recebem um bloco dedicado no `eslint.config.js` com `globals.node` (para `process`, etc.) em vez do `globals.browser` usado em todo o resto — adicione novos scripts exclusivos do Node nessa mesma pasta para que herdem os globais corretos automaticamente.
